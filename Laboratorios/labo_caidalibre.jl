...
# ╔═╡ 1e7ea80a-57af-11ee-3f39-0d6222cae917
using DataFrames, CSV, Plots, DifferentialEquations, Optimization, DiffEqParamEstim,OptimizationOptimJL,Dates, PlutoUI

# ╔═╡ 83c5ca9d-81f8-4e1e-87e0-ac07ace15283
theme(:juno)

# ╔═╡ 45c89371-1db3-4292-9941-4a922f2f6c93
function gravedad!(du,u,p,t)
	g = p[3] 
	du[1] = -u[2]
	du[2] = g - p[1]/p[2]*u[2]*u[2]
end

# ╔═╡ a1ccb158-e070-45ca-b8af-a0962485123d
begin
	h = 55.8
	U0 = [55.8,0]
	U1 = [55.8,0]
...
# ╔═╡ d4a30723-e8db-47c0-adfb-da9307458841
function resolucion_gravedad()
	h = 55.8
	U0 = [55.8,0]
	U1 = [55.8,0]
	m_cañon = 16
	...
	γ_mos = 3.74*10^(-5)
	t = (0.0,10.0)
	cañon  = ODEProblem(gravedad!,U0,t,[γ_cañon,m_cañon,9.81])
	mos = ODEProblem(gravedad!,U1,t,[γ_mos,m_mos,9.81])
	sol_c = solve(cañon)
	sol_m = solve(mos)
end

# ╔═╡ 9faad20c-a104-4aaf-96e4-4e360b36ef25
begin
	s₁ = resolucion_gravedad()
	plot(s₁)
end

# ╔═╡ 90012dba-8e6d-4b46-b35e-ed98ab4019ff
function galileo(u,t,integrator)
	u[1] 
end

# ╔═╡ 4563b7b4-61be-45fe-acc3-69f8b020585c
begin
	cc_galileo    = ContinuousCallback(galileo,nothing,terminate!)
	
	prom_galileo_ca = ODEProblem(gravedad!,U0,t,[γ_cañon,m_cañon,9.81])
	prom_galileo_mos = ODEProblem(gravedad!,U1,t,[γ_mos,m_mos,9.81])

	s_cont_ca = solve(prom_galileo_ca, callback = cc_galileo)
	s_cont_mos = solve(prom_galileo_mos, callback = cc_galileo)
	plot(s_cont_ca, label = ["pos_cañon" "vel_cañon"])
	plot!(s_cont_mos, label = ["pos_mos" "vel_mos"])
end

# ╔═╡ aec49890-8012-4fe3-b725-a62ac6baed92
s_ca_final =s_cont_ca.t[end]

# ╔═╡ 79bfed4a-45a2-46e6-9fcd-18bdbada5187
s_cont_mos.t[end]

# ╔═╡ fbe4c870-08ec-44a1-bc87-ab53cad29e11
s_cont_mos(s_ca_final)[1,]

# ╔═╡ 295c2e3c-32c3-4800-b645-2f395b5c03a5
function pelotita!(du,u,p,t)
	g = p[1] 
	du[1] = -u[2]
	du[2] = g
end

# ╔═╡ e15f0802-50dd-4ee7-b0b4-9f41873e5cd8
function galileo_disc(u,t,integrator)
	u[1] <= 0
end

# ╔═╡ 03844fa6-d3d4-496a-a83d-c9793dee2daa
function rebota!(integrator)
	integrator.u[1] = -integrator.u[1]
	integrator.u[2] = -integrator.u[2]
end

# ╔═╡ 31d63d01-7f0f-4d54-896a-1a85a8b92fa5
function rebota_2!(integrator) 
	integrator.u[1] = -integrator.u[1]
	integrator.u[2] = -0.96integrator.u[2]
end

# ╔═╡ 61ca70c7-bc0b-4372-b192-27096fdff063
begin
	t_2 = (0.0,50.0)
	dc_galileo    = DiscreteCallback(galileo_disc,rebota!)
	prom_pelotita = ODEProblem(pelotita!,U0,t_2,[9.81])
	s_disc_pel = solve(prom_pelotita, callback = dc_galileo,dtmax=0.5)
	plot(s_disc_pel[1,:])
end

# ╔═╡ 98b0315a-3997-4309-8260-b7066485f404
begin
	cc_galileo_p   = ContinuousCallback(galileo,rebota!)
	s_cont_pel = solve(prom_pelotita, callback = cc_galileo_p, dtmax=0.1)
	plot(s_cont_pel[1,:])
end

# ╔═╡ bf49aafe-ec46-48f8-b7c1-b6b30fa1788f
begin
	prom_pelotita_cont = ODEProblem(gravedad!,U0,t_2,[0.1,1,9.81])
	cc_galileo_p_2   = ContinuousCallback(galileo,rebota_2!)
	s_cont_pel_roz = solve(prom_pelotita_cont, callback = cc_galileo_p_2, dtmax=0.1)
	plot(s_cont_pel_roz[1,:])
end

# ╔═╡ 9edc7a5d-06ac-4f0a-ae46-c7527b984e02
function tiro_oblicuo!(du,u,p,t)
	g = p[1] 
	du[1] = u[2]         #x posicion
	du[2] = 0       #x veloidad
	du[3] = -u[4] 	     #y posicion
...
# ╔═╡ 750263da-9c95-4f31-a55f-050215c13eb3
function rebota_roz_ejex!(integrator) 
	if integrator.u[1] >= 2 
		integrator.u[1] = 2
	else 
		integrator.u[1] = -2
	end
	integrator.u[2] = -0.92integrator.u[2]
end

# ╔═╡ 9ad07034-4ccd-40cf-be57-849464861757
function rebota_roz_ejey!(integrator)
	integrator.u[3] = integrator.u[3]
	integrator.u[4] = -0.96integrator.u[4]
end

# ╔═╡ 2176815c-51d4-43a3-b204-654de3efb20d
function rebote_en_x!(u,t,integrator)
	u[1] >= 2 || u[1] <= -2
end

# ╔═╡ 2abf39c3-0090-420e-98cc-03b51543e294
function rebote_en_y!(u,t,integrator)
	u[3] <= 0 
end

# ╔═╡ c2be75bf-cf36-4d2a-aa54-37ada6bb303f
begin
	in0 = [0,0.5,15,0]
	dc_con_paredes_x    = DiscreteCallback(rebote_en_x!,rebota_roz_ejex!)
	dc_con_paredes_y    = DiscreteCallback(rebote_en_y!,rebota_roz_ejey!)

	dc_con_paredes = CallbackSet(dc_con_paredes_x,dc_con_paredes_y)
	
	prom_paredes = ODEProblem(tiro_oblicuo!,in0,t_2,[9.81])
	
	
	sol_paredes_disc = solve(prom_paredes, callback = dc_con_paredes,dtmax=0.04)
end

# ╔═╡ 20a97963-9178-4285-a450-7e491aa4e121
plot(sol_paredes_disc.t,sol_paredes_disc[1,:])

# ╔═╡ 6cd33cc3-10dc-4644-b794-39584fdcbd75
begin
	xx = sol_paredes_disc[1,:]
	yy = sol_paredes_disc[3,:]
end

# ╔═╡ ce0f5fb4-889c-4dd9-accd-77cb7d95e92e
scatter(xx,yy)

# ╔═╡ 2cd716a7-67a8-48ac-9ffd-258c28b643a2
begin 
	xrect_2 = [2,2]
	xrect_y2 = [-1,15]
...
scatter([xx[i]],[yy[i]],xlims=(-3,3),ylims=(-1,15), label = "posicion")
	plot!(xrect_2, xrect_y2 )
	plot!(xrect_m2, xrect_ym2)
	
end
end

# ╔═╡ 37b6449d-0f85-48f2-be13-5cb875b7f8cc
mp4(anim,fps=35)

# ╔═╡ be426769-3add-42e7-8a4e-fc1eca462eb4
mp4(anim, "pelotita.mp4",fps =24)
