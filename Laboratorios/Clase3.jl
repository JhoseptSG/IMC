...
# ╔═╡ c51e4fd0-4512-11ee-3ed7-a91d3235fe65
using Plots

# ╔═╡ a18b2fd3-0a3b-4461-9610-62d4238f0c05
using PlutoUI

# ╔═╡ ee659819-cabb-4f50-8b38-562b74baab4b
using DifferentialEquations

# ╔═╡ 421b6502-5cda-4b5d-8fba-a5c18d09ecf2
...
# ╔═╡ 659a3ca0-a53d-4670-8c3b-f0f447bd61ef
 function euler_vec(f, t0, tf, u0, h)
        ...
        for i in 1:n-1
            u[:, i+1] .= u[:, i] .+ h .* f(u[1, i], u[2, i])
        end

        return t, u
    end

# ╔═╡ a4b323e0-3827-405c-b321-7f53fff4055f
# Ejercicio 7 - depredador - presa 

# ╔═╡ 3d9ca478-1c00-4d9b-9730-831415241f9f
#defino variables
begin
	a = 0.25;
	B = 1;
...
# ╔═╡ c52727d0-33b7-4418-9135-bb76046e4057
...
function sistema(x, y)
    return [dx(x, y), dy(x, y)]
end 
end

# ╔═╡ 06d2d594-f385-46bf-adb5-db66d212bba2
# a) tiene equilibrio en y = a / l , x = B / q  y (0,0)
# 	 pues dx = dy = 0 

# ╔═╡ dd5e4bc7-82a5-413a-afa9-0b4c63010e51
@bind x0 Slider(0:50)

# ╔═╡ 362c5c32-2eb9-48d8-b3e7-34527d115373
@bind y0 Slider(0:250)

# ╔═╡ 4a6135d1-9c0a-4e32-ac4a-11368255e134
#b) resolver el sistema de ecuaciones
begin
	pob_0 = [x0,y0] ;  # cerca del (1,1)
 	tiempo, poblacion = euler_vec(sistema, 0, 5, pob_0, 0.01);
end

# ╔═╡ cee93692-baa0-4361-846f-f8390145ac32
begin
plt1 = plot(tiempo,poblacion[1,:])
plot(plt1,tiempo,poblacion[2,:])
end

# ╔═╡ 2482f22c-d53f-4fbe-86b6-5dcedc084ee4
begin
	plt = plot()
	plot!(plt,poblacion[2,:],poblacion[1,:])
	xlabel!("presas")
	ylabel!("depredadores")
	plt
end

# ╔═╡ d5e07f39-bb3f-4133-a0be-c780cb38ae86
function predpresa(u,p,t)
	
end 

# ╔═╡ f79ee9a6-cf7c-4c34-8158-2566087ec686
begin
...
u0 = [x0, y0]  # Define tus propias condiciones iniciales

# Rango de tiempo
tspan = (0.0, 5)  # Define tu propio rango de tiempo

# Define el problema de ecuaciones diferenciales
prob = ODEProblem(predpresa, u0, tspan, params)

# Resuelve el problema utilizando el método de integración
sol = solve(prob)

# Visualiza la solución
plot(sol, xlabel="Tiempo", ylabel="Valor", label=["x" "y"])
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "c1387483123883f342fb0c880dcd249c89efb9e4"
