...
# ╔═╡ 64bc760c-522e-11ee-1af2-6352ec81e824
using DifferentialEquations,DiffEqParamEstim,Plots, CSV, DataFrames,Optimization, OptimizationOptimJL

# ╔═╡ 72e9f5ba-defd-47ff-aa4f-485faf6dbde0
df = CSV.read("/home/clinux01/Descargas/influenza_2022_2023.csv",DataFrame)

# ╔═╡ 05a964e1-ce9f-4db4-852b-a6945fc1676a
function SIR!(du,u,p,t)
	B,l = p
	du[1] = -B*u[1]*u[2]
	du[2] = B*u[1]*u[2]-l*u[2]
	du[3] = l*u[2]
end

# ╔═╡ 406584b6-0c0b-477d-9863-7b04c6cfe338
total_a

# ╔═╡ f606b118-89cf-4da5-8945-f5699f03fd2d
begin
	total_a = df[!,"Percent Positive A"]/100 
	total_b = df[!,"Percent Positive B"]/100
	total = df[!,"% Positive"]/100
	info = [total_a,total_b,total]
	tspan = [1,42]
	t = 1:42
end	
...
# ╔═╡ 4574b877-899a-4ca7-9308-716c501b228b
function loss(sol,data) #reemplaza el L2loss
if any((s.retcode != :Success for s in sol)) #que sea succes,sino infinito
return Inf #esto es para que aborte si hay problemas en el solver. retornamos infinito cuando los parametros son tan malos que la ec.diferencial no se puede resolver
else
return sum((sol.(t,idxs=coord_data)-data).^2) #hace la cuenta del error que quiero calcular. evaluo la solucion en t y le digo que me devuelva solamente la segunda coordenada
#+ 1000min(0,sol[coord_data+1,1])^2
#hago lo miso que hae L2loss pero solo en la segunda coordenada
end
end

# ╔═╡ 9f56f5ce-19da-4e5d-beb6-e9666134037a
# ╠═╡ disabled = true
#=╠═╡
begin
	prob_SIR = []
	...
	for i in 1:3
		push!(in0,info[i][1])
		push!(prob_SIR,ODEProblem(SIR!,in0[i],tspan,[0.01]))
		push!(func_SIR,
			build_loss_objective(prob_SIR[i],AutoTsit5(Rosenbrock23()),sol ->loss(sol,info[i]),Optimization.AutoFiniteDiff()))

		push!(opt_prob_SIR, OptimizationProblem(func_SIR[i],0.1rand(2),lb=zeros(2),ub=ones(2)))
		push!(sol_SIR,solve(opt_prob_SIR[i],SAMIN(),maxiters=5000))
	end
